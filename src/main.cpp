...
int main(int argc, char *argv[]) {
  ...
  switch (num_devices) {
  /**************************************************************
  // CASE 1: 1 CAMERA REALSENSE DETECTED
  ***************************************************************/
  case 1: {
    std::cout << "There are " << num_devices << " connected RealSense devices." << std::endl;

    rs::device &_rs_camera = init_realsense_camera(rs_ctx, 0);
    ...
    while (!viewer->wasStopped()) {
      if (_rs_camera.is_streaming()) {
        ...
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr points_camera = points_to_pcl(_rs_camera);

        pcl::PointCloud<pcl::PointXYZRGB>::Ptr points_camera_backup(
            new pcl::PointCloud<pcl::PointXYZRGB>);

        *points_camera_backup = *points_camera;

        if (cont <= 0) {

          std::string str = "Points: ";
          std::stringstream ss;
          ss << points_camera->points.size();
          str += ss.str();

          viewer->addPointCloud(points_camera, "points_camera");
          ...
          viewer->updatePointCloud(points_camera, "points_camera");
  ...
  /**************************************************************
  // CASE 2: 2 CAMERAS REALSENSE DETECTED
  ***************************************************************/
  case 2: {
    std::cout << "There are " << num_devices << " connected RealSense devices." << std::endl;
    ...
    while (!viewer->wasStopped()) {
      ...
      if (_rs_camera1.is_streaming() and _rs_camera2.is_streaming()) {
        ...
        *points_camera2_backup = *points_camera2;
        ...
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr points_camera_fusion(
            new pcl::PointCloud<pcl::PointXYZRGB>());
        ...
        std::cout << "-> Descriptors:			" << camera1_descriptors->points.size()
                  << std::endl;
        std::cout << std::endl;
        ...
        std::cout << "-> Descriptors:			" << camera2_descriptors->points.size()
                  << std::endl;
        std::cout << std::endl;
        ...
        pcl::PointCloud<pcl::PointXYZI>::Ptr sac_registration(
            new pcl::PointCloud<pcl::PointXYZI>());
        ...
        Eigen::Isometry3f pose(
            Eigen::Isometry3f(Eigen::Translation3f(transformed_target_cloud->sensor_origin_[0],
        ...
        // now both point clouds are transformed to the origin and can be
        // added up
        ...
        *output_cloud += *transformed_target_cloud;
        ...
        // pcl::transformPointCloud(*points_camera1_backup,
        // *camera2alignetocamera1, transformation_matrix);*/
        ...
        if (cont <= 0) {
          ...
          pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> color_handler(
              points_camera1_backup, 255, 255, 255);
          ...
          pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> color_handler1(
              points_camera1, 255, 0, 0);
...
*/
  ...
  default:
    ...
    pcl::console::print_warn("No devices detected. Is it plugged in?\n");
    pcl::console::print_warn("Min: 1 camera, Max: 2 cameras\n");

    // std::cout << "No devices detected. Is it plugged in?" << std::endl;
    // std::cout << "Min: 1 camera, Max: 2 cameras" << std::endl;
    break;
  } // switch case end
