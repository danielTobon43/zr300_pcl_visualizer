...
int main(int argc, char *argv[]) {
  ...
  std::cout << "\nLoaded file " << argv[1] << " (" << cloud_tr->size() << " points) in "
            << time.toc() << " ms\n"
            << std::endl;
  ...
  *cloud_icp = *cloud_tr;
  ...
  std::cout << "Applied " << iterations << " ICP iteration(s) in " << time.toc() << " ms"
            << std::endl;
  ...
  pcl::visualization::PointCloudColorHandlerCustom<PointT> cloud_icp_color_h(cloud_icp, 180, 20,
                                                                             20);
    ...
    if (next_iteration) {
      ...
      if (icp.hasConverged()) {
        ...
        transformation_matrix *=
            icp.getFinalTransformation().cast<double>(); // WARNING /!\ This is not accurate! For
                                                         // "educational" purpose only!
        print4x4Matrix(transformation_matrix); // Print the transformation between original pose and
                                               // current pose
